...
function renderResults(location, results) {
  ...
  //Fékk hjálp frá chatgpt
  const rows = results.map((forecast) =>
    el(
      "tr",
      {},
      el("td", {}, forecast.time.split("T")[1]),
  ...
  renderIntoResultsContent(
    el(
      ...
      el(
        "p",
        {},
        `Spa fyrir daginn á breiddargráðu ${location.lat}
...
// Hnit notanda ef allt er rétt
function showPosition(position) {
  const latitude = position.coords.latitude;
  const longitude = position.coords.longitude;

  // Uppfærir Mín staðsetning í hnit notanda
  const myLocation = locations.find(
    (location) => location.title === "Mín staðsetning (þarf leyfi)"
  );
  if (myLocation) {
